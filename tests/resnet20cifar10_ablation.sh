...
for N_PRE in "${N_PRE_VALS[@]}"; do
  ...
  for N_ITER in "${N_ITER_VALS[@]}"; do
    ...
    --num_pre_prune_epochs=$N_PRE --num_prune_iterations=$N_ITER \
    --num_train_epochs_before_pruning=10 --num_prune_epochs=$N_EPOCH_PRUNE \
    --num_train_epochs=300 --checkpoint_dir="../checkpoints_ablation/ prune_amount_0.98/N_PRE_${N_PRE}"
    done
  done
done
...
for N_PRE in "${N_PRE_VALS[@]}"; do
  ...
  for N_ITER in "${N_ITER_VALS[@]}"; do
    ...
    --num_pre_prune_epochs=$N_PRE --num_prune_iterations=$N_ITER \
    --num_train_epochs_before_pruning=10 --num_prune_epochs=$N_EPOCH_PRUNE \
    --num_train_epochs=300 --checkpoint_dir="../checkpoints_ablation/ prune_amount_0.9/N_PRE_${N_PRE}"
    done
  done
done